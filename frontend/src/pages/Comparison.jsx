...
import { ArrowLeft, AlertCircle, CheckCircle2, GitCompare, IndianRupee } from 'lucide-react';

export default function Comparison() {
  const { state } = useLocation();
  const navigate = useNavigate();

  if (!state?.colleges || state.colleges.length < 2) {
    return (
  ...
  const { colleges, exam } = state;
  const [c1, c2] = colleges;

  const renderBetter = (val1, val2, lowerIsBetter = false) => {
    if (val1 == null || val2 == null) return null;
    if (val1 === val2) return null;
    const isC1Better = lowerIsBetter ? val1 < val2 : val1 > val2;
    return isC1Better ? 0 : 1;
  };

  const c1BetterMark = <CheckCircle2 size={16} className="text-green-500 inline-block ml-1" />;
  ...
  const comparisonRows = exam === 'eapcet' ? [
    ...
    { label: 'Tuition Fee (Lower is better)', 
      v1: c1.tuition_fee ? `₹${c1.tuition_fee.toLocaleString()}` : '-', 
      v2: c2.tuition_fee ? `₹${c2.tuition_fee.toLocaleString()}` : '-',
      better: renderBetter(c1.tuition_fee, c2.tuition_fee, true)
    },
    { label: 'Est. ROI (Higher is better)', 
      v1: c1.roi ? `${c1.roi}x` : '-', 
      v2: c2.roi ? `${c2.roi}x` : '-',
      better: renderBetter(c1.roi, c2.roi, false)
    },
    { label: 'Est. Avg Package', v1: '₹4,50,000', v2: '₹4,50,000' }
  ] : [
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
       <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 mb-6 transition-colors">
      ...
      <div className="card overflow-hidden">
        <div className="grid grid-cols-3 bg-slate-50 border-b border-slate-100 divide-x divide-slate-100">
          ...
          <div className="p-4 text-center">
            ...
            <button onClick={()=>navigate(`/${exam}/college/${c1._id}`)} className="text-xs text-blue-600 mt-2 hover:underline">View Details</button>
          </div>
          <div className="p-4 text-center">
            ...
            <button onClick={()=>navigate(`/${exam}/college/${c2._id}`)} className="text-xs text-blue-600 mt-2 hover:underline">View Details</button>
          </div>
        </div>

        <div className="divide-y divide-slate-100">
          {comparisonRows.map((row, idx) => (
             <div key={idx} className="grid grid-cols-3 divide-x divide-slate-100 hover:bg-slate-50 transition-colors">
               <div className="p-4 flex items-center text-sm font-medium text-slate-600">{row.label}</div>
               <div className={`p-4 flex items-center justify-center text-center text-sm ${row.better === 0 ? 'bg-green-50/50 font-semibold text-green-800' : 'text-slate-800'}`}>
                 {row.v1} {row.better === 0 && c1BetterMark}
               </div>
               <div className={`p-4 flex items-center justify-center text-center text-sm ${row.better === 1 ? 'bg-green-50/50 font-semibold text-green-800' : 'text-slate-800'}`}>
                 {row.v2} {row.better === 1 && c1BetterMark}
               </div>
             </div>
          ))}
        </div>
