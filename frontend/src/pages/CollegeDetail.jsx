...
export default function CollegeDetail() {
  ...
  const exam = pathname.includes('/eapcet/') ? 'eapcet' : 'josaa';
  ...
  useEffect(() => {
    const fetchCollege = async () => {
      try {
        ...
        const res = await api.getCollege(id);
        setCollege(res.data);
      } catch (err) {
  ...
  if (!college) return <div className="p-12 text-center text-slate-500">College not found</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 transition-colors">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
             ...
             <div className="flex items-center gap-4 text-sm text-slate-500">
               <span className="flex items-center gap-1"><MapPin size={16}/> {place}</span>
               {exam === 'josaa' && <span className="flex items-center gap-1"><Building2 size={16}/> {college.institute_type}</span>}
               {exam === 'eapcet' && college.affiliated_to && <span className="flex items-center gap-1"><Shield size={16}/> {college.affiliated_to}</span>}
             </div>
          </div>
         ...
         <div className="card p-6">
           ...
           <div className="space-y-4">
             {exam === 'eapcet' && (
               <>
                 ...
                 <div>
                   ...
                   <div className="text-sm font-medium text-slate-800">{college.college_type} ({college.co_education})</div>
                 </div>
             ...
             {exam === 'josaa' && (
               <>
                  <div>
                   <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Quota & Seat Type</div>
                 ...
                 <div>
                   <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Last Round Cutoff</div>
            ...
            {exam === 'eapcet' && (
               <div className="max-h-[250px] overflow-y-auto pr-2">
                 <table className="w-full text-sm text-left text-slate-500">
                   ...
                   <tbody>
                     {['OC','BC_A','BC_B','BC_C','BC_D','BC_E','SC','ST','EWS'].map(cat => {
                        const bgKey = cat === 'EWS' ? 'EWS_GEN_OU' : `${cat}_BOYS`;
                        const ggKey = cat === 'EWS' ? 'EWS_GIRLS_OU' : `${cat}_GIRLS`;
                        const bVal = college.cutoffs?.[bgKey];
                        const gVal = college.cutoffs?.[ggKey];
                        if (!bVal && !gVal) return null;
                        return (
                          <tr key={cat} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                            <td className="px-3 py-2 font-semibold text-slate-700">{cat}</td>
                            <td className="px-3 py-2">{bVal?.toLocaleString() || '-'}</td>
                            <td className="px-3 py-2">{gVal?.toLocaleString() || '-'}</td>
                          </tr>
                        );
                     })}
                   </tbody>
              ...
              <div className="h-[250px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={college.round_trends} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                    ...
                    <XAxis dataKey="round" tickFormatter={(v) => `R${v}`} tick={{fontSize: 12, fill: '#64748b'}} axisLine={false} tickLine={false} />
                    <YAxis reversed domain={['auto', 'auto']} tick={{fontSize: 12, fill: '#64748b'}} axisLine={false} tickLine={false}/>
                    <Tooltip 
                      contentStyle={{borderRadius: '8px', border:'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)'}}
                      labelFormatter={(v) => `Round ${v}`}
                      formatter={(val) => [val.toLocaleString(), 'Closing Rank']}
                    />
                    <Line type="monotone" dataKey="closing_rank" stroke="#8b5cf6" strokeWidth={3} dot={{r:4, fill:'#8b5cf6', strokeWidth:2, stroke:'#fff'}} activeDot={{r:6}} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
